...
const App = (function() {
  ...
  async function loadUserData() {
    ...
    if (addressElement) {
      ...
      // Function to generate fallback address if needed
      const generateFallbackAddress = () => {
        return '0x' + Array.from({length: 40}, () => 
          Math.floor(Math.random() * 16).toString(16)
        ).join('');
      };
      
      // If user and blockchainAddress exist, display it
      if (user && user.blockchainAddress) {
        ...
        try {
          ...
          if (result.success && result.address) {
            ...
            if (user) {
              Auth.updateCurrentUser({ blockchainAddress: result.address });
            }
          } else {
            // Use fallback address
            const fallbackAddress = generateFallbackAddress();
            addressElement.textContent = fallbackAddress;
            
            // Store fallback address
            if (user) {
              Auth.updateCurrentUser({ blockchainAddress: fallbackAddress });
            }
          }
        } catch (error) {
          console.error('Error getting address:', error);
          // Use fallback address even in error case
          const fallbackAddress = generateFallbackAddress();
          addressElement.textContent = fallbackAddress;
